...
func main() {
	...
	// CORS configuration with a regex validator for allowed origins
	allowedOrigins := handlers.AllowedOriginValidator(func(origin string) bool {
		// Allow koopfon.com, its subdomains, Render preview URLs, and localhost
		matched, _ := regexp.MatchString(`^https?://(localhost|(.+\.)?koopfon\.com|(.+\.)?onrender\.com)(:\d+)?package main
